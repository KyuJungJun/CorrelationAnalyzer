...
#             self.neighbor_sets_matrix = np.transpose(np.array(self.neighbor_sets_matrix))
    ...
    def count_rotations_single_dt(self, dt, min_peak_height, peak_width=None):
        ...
        for target_index, arr in enumerate(self.rotations_each_time):
            peak, info = scipy.signal.find_peaks([i[num_dt_frames]*180*2/np.pi for i in arr], height=min_peak_height, width=peak_width)
            peak_dict = [{"time":peak[i]*20, "height":info['peak_heights'][i]
